...
To run the integration tests in `CdcSourceIntegrationTests` you need to start the source's database Docker container as explained below:
...
<3> When `true` includes the https://debezium.io/docs/connectors/mysql/#change-events-value[Change Event Value] schema.
