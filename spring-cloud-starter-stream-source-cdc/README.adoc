...
cdc.config.database.history=io.debezium.relational.history.MemoryDatabaseHistory <1>
...
<2> Required property that configures the CDC Source to use `PostgresConnector`.
<3> Required properties to connection to the PostgreSQL on `localhost:5432` as `postgres` user.
