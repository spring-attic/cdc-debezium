...
[source]
----
cdc.config.connector.class=io.debezium.connector.mysql.MySqlConnector # <1>

cdc.config.database.user=debezium  # <2>
cdc.config.database.password=dbz # <2>
cdc.config.database.hostname=localhost # <2>
cdc.config.database.port=3306 # <2>

cdc.includeSchema=true # <3>
cdc.filtering.enabled=true # <4>
----

<1> Required property that configures the CDC Source to use `MySqlConnector`.
<2> Required properties to connection to the MySql on `localhost:3306` as `debezium`.
<3> Includes the https://debezium.io/docs/connectors/mysql/#change-events-value[Change Event Value] schema in the `SourceRecord` events.
<4> Enables the https://debezium.io/docs/configuration/event-flattening/[CDC Event Flattering].
...
[source]
----
cdc.config.connector.class=io.debezium.connector.postgresql.PostgresConnector # <1>

cdc.config.database.user=postgres  # <2>
cdc.config.database.password=postgres # <2>
cdc.config.database..dbname=postgres # <2>
cdc.config.database.hostname=localhost # <2>
cdc.config.database.port=5432 # <2>

cdc.includeSchema=true # <3>
cdc.filtering.enabled=true # <4>
----

<1> Required property that configures the CDC Source to use `PostgresConnector`.
<2> Required properties to connection to the PostgreSQL on `localhost:5432` as `postgres` user.
<3> Includes the https://debezium.io/docs/connectors/mysql/#change-events-value[Change Event Value] schema in the `SourceRecord` events.
<4> Enables the https://debezium.io/docs/configuration/event-flattening/[CDC Event Flattering].
