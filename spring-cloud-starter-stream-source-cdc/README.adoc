...
All Debezium engine and connector configuration properties are supported. Just apply them with `cdc.config.` property prefix. For example for setting the  `connector.class` use the `cdc.config.connector.class` property.

For the most frequently used properties we are providing a `springified` shortcuts with auto-completion support. For example instead of `cdc.config.connector.class=io.debezium.connector.mysql.MySqlConnector` you can use `cdc.connector=mysql` instead!
When both the `cdc.config.XXX` and its shortcut version are set for the same property are `cdc.config.` setting has precedence!
